...
data State =
  S Coord Direction Boxes MazeMap [Integer] [Integer] Integer Integer LevelName
...
data Interaction world =
  Interaction world (Event -> world -> world) (world -> SizedPicture)
...
data Maze =
  Maze Coord MazeMap Boxes LevelName
...
startScreen = lettering "Sokoban!" & translated 0 2 (lettering informationStart)
...
handlePlaying (KeyPress key) s
  ...
  | key == "d" = w' R
  where
    w' x = controlledAdjacentCoord x s
handlePlaying _ s = s
...
withUndo (Interaction state0 handle drawing) =
...
runInteraction i = interactionOf w e d
  where
    (Interaction w e d) = (resettable . withStartScreen . withUndo) i
...
type Interactive world = world -> EventHandler world -> Drawer world -> IO ()
