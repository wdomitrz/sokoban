...
-- typ współrzędnych, Eq przdatne do między innymi delete
...
charToTile c 
  | c == '#'  = Wall
  | c == '@'  = Ground -- Różne konwencje gracza na podłocze
  | c == 'p'  = Ground -- Różne konwencje gracza na podłocze
  | c == '+'  = Storage  -- Gracz na polu końcowym
  | c == '$'  = Ground   -- Różne konwencje pudełka (pudełko na podłodze)
  | c == 'b'  = Ground   -- Różne konwencje pudełka (pudełko na podłodze)
  | c == '*'  = Storage -- Różne konwencje pudełka (pudełko na storage) NOWOŚĆ!
  | c == '.'  = Storage
  | c == ' '  = Ground -- Różne koncepcje podłogi
  | c == '-'  = Ground -- Różne koncepcje podłogi
  | c == '_'  = Blank -- Pole, które na pewno będzie puste
  | otherwise = Blank
...
-- Poruszenie współrzędnej do górys
upCoord :: Coord -> Coord
...
foldTable modifyValue initialValue = 
  ...
  in snd . foldl  
            (\acc@(c, _) row -> (upCoord c, snd $ foldRow acc row))
            (C 0 0, initialValue)

-- Parser poziomów (stosuję odpowiednio foldl i foldr, żeby zachować kolejność)
mazeParser :: (Map, LevelName) -> Maze
mazeParser (mapPicture, lvlName) = (Maze (plr mapPicture) (createMap mapPicture) (getBoxes mapPicture) lvlName)
  where plr :: Map -> Coord
        plr table =
        ...
        createMap table =
          let tableToFunction :: Map -> (Coord -> Char)
              tableToFunction = foldTable (\c el res -> (\x -> if x == c then el else res x)) (\_ -> '_')
...
someDim takeSome (Maze c mm _ _) =
  let canWalk :: Tile -> Bool
      canWalk x = (x /= Blank)

      neighbours :: Coord -> Boxes
      neighbours v = foldr (\d acc -> 
                            let nv = adjacentCoord d v
                            in if canWalk $ mm nv 
                                then nv:acc 
                                else acc) [] directions
...
isWinning (S _ _ b mm _ _ _ _ _) = 
  allList 
    (\c ->
...
initialWorld n m =
  S (initialCoord m) (initialPlayerDirection m) (initialBoxes m) (initialMap m) (xDim m) (yDim m) n 0 (initalName m)
...
startScreen = 
  lettering "Sokoban!" & 
  translated 0 2 (lettering informationStart)
...
nextLevel mazes n nm ln = lettering ("Level finished, number of moves: " ++ (show nm) ++ ", level name: " ++ ln) &
...
youWon mazes n nm ln =  lettering "YOU WON!!!" & 
                        translated 0 2 (lettering ("Number of moves: " ++ (show nm) ++ ", last level: " ++ ln) &
...
drawMazeElement mm x y =
  translated x y $ drawTile $ mm (C x y)
...
drawPlaying :: Draw
drawPlaying (S c d b mm xd yd _ _ _) = atCoord c (player d) & pictureOfBoxes b & pictureTheMazeInRange mm xd yd
...
draw mazes s = 
  let how :: Draw
      how
        | ls == LevelWon  = drawNextLevel mazes
...
handleNextLevel mazes (KeyPress key) (S _ _ _ _ _ _ n _ _)
  | key == "n" = initialWorld (n + 1) (nth mazes n)
handleNextLevel _ _ s = s

-- obsługa przyciśnięć klawiszy w trakcie gry (na razie bez strzałek)
handlePlaying :: Handler
handlePlaying (KeyPress key) s
  | key == "w" = w' U
  | key == "s" = w' D
  | key == "a" = w' L
  | key == "d" = w' R
...
handleLevelReset mazes _ (KeyPress key) (S _ _ _ _ _ _ n _ _)
  | key == "r" = initialWorld n (nth mazes (n-1))
...
handleEvent mazes e s = 
  let how :: Handler
      how
        | ls == LevelWon  = handleNextLevel mazes
...
controlledAdjacentCoord d s@(S c _ b mm xd yd n mn ln) = 
  let helper :: Tile -> State
      ...
      helper Box = 
        ...
        where nbs = (S nc d nb mm xd yd n (mn+1) ln)
  ...
  where mazeWithBoxes = addBoxes b mm
...
withUndo (Interaction state0 handle drawing) = Interaction state0' handle' drawing' where
...
withStartScreen (Interaction state0 handle drawing)
  = Interaction state0' handle' drawing'
  where
    state0' = StartScreen
...
resettable (Interaction state0 handle drawing)
  = Interaction state0 handle' drawing
  where handle' (KeyPress key) _ | key == "p" = state0
...
drawFunSizedToScreen (range@(xSize, _), toDraw) =
  map (\(x, y) -> if x == xSize then '\n' 
                                else toDraw x y) 
...
translated xt yt (range, picture) = (translateRange xt yt range, translatedPicture xt yt picture)
...
lettering l = 
  ((ll, 1), 
    \f x y -> if y /= 0 || x < 0 || x >= ll 
              then f x y
              else nth l (ll - 1 - x)) -- Można efektywniej, ale wystarczy
